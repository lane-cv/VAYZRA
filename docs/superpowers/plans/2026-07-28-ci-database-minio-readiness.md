...
```bash
go_test_line="$(exact_line "      - run: GOENV=off GOFLAGS='' go test -p 1 ./... -count=1")"
go_race_line="$(exact_line "      - run: GOENV=off GOFLAGS='' go test -race -p 1 ./... -count=1")"
...
Expected: FAIL with `missing verify-job workflow line:       - run: GOENV=off GOFLAGS='' go test -p 1 ./... -count=1`.
...
```yaml
      - run: GOENV=off GOFLAGS='' go test -p 1 ./... -count=1
      - run: GOENV=off GOFLAGS='' go test -race -p 1 ./... -count=1
```
