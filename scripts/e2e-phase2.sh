...
docker_bounded 1200 run --rm --name "$e2e_runner" --network "$network" --read-only --user 1000:1000 --shm-size 512m --memory 1280m --cpus .6 --cap-drop ALL --security-opt no-new-privileges --tmpfs /tmp:rw,noexec,nosuid,size=256m \
  ...
  -e "E2E_STUDENT_NEW_PASSWORD=$student_new_password" -e E2E_FIXTURE_DIR=/fixtures -e E2E_OUTPUT_DIR=/artifacts/results/phase2 \
