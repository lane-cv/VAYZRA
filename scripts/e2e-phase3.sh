...
diagnostics() {
  ...
  find "$artifact_dir" -type f \( -name 'trace.zip' -o -name '*.png' -o -name '*.webm' \) -delete
}
...
docker run --rm --network none --user 0:0 --entrypoint /bin/sh -v "$runner_volume:/workspace" "$playwright_image" -c 'chown 1000:1000 /workspace && chmod 0700 /workspace'
...
docker run --rm --network none --user 0:0 --memory 768m --cpus .5 --entrypoint /bin/bash -v "$PWD:/src:ro" -v "$fixture_volume:/fixtures" "$worker_image" \
  /src/scripts/generate-phase2-fixtures.sh /fixtures
docker run --rm --read-only --user 1000:1000 --memory 1280m --cpus .6 --tmpfs /tmp:rw,noexec,nosuid,size=64m \
...
docker run --rm --network "$network" --read-only --user 1000:1000 --shm-size 512m --memory 2048m --cpus 1 \
