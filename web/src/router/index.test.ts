...
describe('console router guards', () => {
  ...
  it('allows only admins to open canonical teacher question routes',async()=>{const session=useSessionStore();session.bootstrapStatus='ready';session.user={id:'a1',username:'teacher',displayName:'张老师',role:'admin',mustChangePassword:false};const router=createAppRouter();await router.push('/admin/questions');expect(router.currentRoute.value.name).toBe('admin-questions');await router.push('/admin/questions/not-a-uuid');expect(router.currentRoute.value.name).toBe('admin-questions');await router.push('/admin/questions/11111111-1111-4111-8111-111111111111');expect(router.currentRoute.value.name).toBe('admin-question-detail');session.user={id:'s1',username:'student',displayName:'学生',role:'student',mustChangePassword:false};await router.push('/admin/questions');expect(router.currentRoute.value.fullPath).toBe('/student')})
