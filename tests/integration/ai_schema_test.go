...
func TestAISchemaHistoryAndRunConstraints(t *testing.T) {
	...
	if _, err := pool.Exec(ctx, aiRunInsertSQL, uuid.New(), threadID, student, studentMessage, 2, "ai-run-key-00000002", "queued", config.providerID, config.modelID, config.promptID); err == nil {
		t.Fatal("second active run for student succeeded")
	}
	if _, err := pool.Exec(ctx, aiRunTerminalInsertSQL, uuid.New(), threadID, student, studentMessage, 2, "ai-run-key-00000001", "failed", config.providerID, config.modelID, config.promptID); err == nil {
		t.Fatal("duplicate student idempotency key succeeded")
	}
	if _, err := pool.Exec(ctx, aiRunTerminalInsertSQL, uuid.New(), threadID, student, studentMessage, 1, "ai-run-key-00000003", "failed", config.providerID, config.modelID, config.promptID); err == nil {
	...
	if _, err := pool.Exec(ctx, `UPDATE ai_runs SET status='streaming',lease_owner='worker-1',lease_expires_at=now()+interval '1 minute',heartbeat_at=now(),started_at=now() WHERE id=$1`, runID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `UPDATE ai_runs SET status='succeeded',lease_owner=NULL,lease_expires_at=NULL,heartbeat_at=NULL,completed_at=now(),usage_source='upstream',input_tokens=10,output_tokens=20,cost_micro_usd=3 WHERE id=$1`, runID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO ai_messages(id,thread_id,role,body_text,trigger_run_id) VALUES($1,$2,'assistant','answer',$3)`, assistantID, threadID, runID); err != nil {
		t.Fatal(err)
	}
...
func assertAIMessageAcceptsAssistantWithoutUserID(t *testing.T, pool *pgxpool.Pool) {
	...
	if _, err := pool.Exec(context.Background(), `UPDATE ai_runs SET status='failed',completed_at=now(),error_code='test_failure' WHERE id=$1`, runID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(context.Background(), `UPDATE ai_runs SET status='succeeded',lease_owner=NULL,lease_expires_at=NULL,heartbeat_at=NULL,completed_at=now(),usage_source='estimated',input_tokens=1,output_tokens=1,cost_micro_usd=0 WHERE id=$1`, runID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(context.Background(), `INSERT INTO ai_messages(id,thread_id,role,body_text,trigger_run_id) VALUES($1,$2,'assistant','completed answer',$3)`, uuid.New(), threadID, runID); err != nil {
...
func aiRun(t *testing.T, pool *pgxpool.Pool, threadID, student, messageID uuid.UUID, attempt int, key string, config aiConfigIDs) uuid.UUID {
	t.Helper()
	id := uuid.New()
	if _, err := pool.Exec(context.Background(), aiRunInsertSQL, id, threadID, student, messageID, attempt, key, "queued", config.providerID, config.modelID, config.promptID); err != nil {
		t.Fatal(err)
	}
	return id
}
...
INSERT INTO ai_runs(
  ...
  $10,'math',1,'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
