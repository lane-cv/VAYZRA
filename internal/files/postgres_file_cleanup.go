...
WHERE f.deleted_at IS NOT NULL AND fv.purged_at IS NULL AND fv.retention_until<=$1
...
FOR UPDATE OF f,fv SKIP LOCKED LIMIT 1`, now).Scan(&candidate.FileID, &candidate.VersionID, &candidate.OriginalKey, &actorID)
 ...
 SELECT 'file.cleanup',jsonb_build_object('fileId',$1,'fileVersionId',$2)
...
func fileReferenced(ctx context.Context, tx pgx.Tx, fileID uuid.UUID) (bool, error) {
	var referenced bool
	err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM lesson_draft_files b JOIN file_versions x ON x.id=b.file_version_id WHERE x.file_id=$1) OR EXISTS(SELECT 1 FROM lesson_revision_files b JOIN file_versions x ON x.id=b.file_version_id WHERE x.file_id=$1)`, fileID).Scan(&referenced)
