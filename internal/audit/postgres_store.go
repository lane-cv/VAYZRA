...
func systemActorAllowed(event Event) bool {
	switch event.Action {
	case "operations.lease_taken_over":
		return event.TargetType == "operational_mode" &&
			event.TargetID == "global" &&
			len(event.Metadata) == 0 &&
			event.RequestID == "operations-lease-takeover" &&
			event.IP.Equal(net.ParseIP("127.0.0.1"))
	case "operations.retention_completed":
		return event.TargetType == "metadata_retention" &&
			event.TargetID == "global" &&
			event.RequestID == "operations-retention" &&
			event.IP.Equal(net.ParseIP("127.0.0.1"))
	default:
		return false
	}
...
func validOperationsEvent(event Event) bool {
	...
	case "operations.retention_completed":
		if event.ActorUserID != uuid.Nil || len(event.Metadata) != 5 {
			return false
		}
		for _, key := range []string{
			...
			if err != nil || count < 0 || strconv.FormatInt(count, 10) != value {
				return false
			}
		}
		return true
